<?php
$title = "Xóa Heo Trong Trại";

// === XỬ LÝ XÁC NHẬN XÓA - PHẢI ĐẶT TRƯỚC MỌI THỨ ===
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    // Lấy mã heo từ URL (ví dụ: index.php?url=heo/delete&maHeo=H006)
    $maHeo = $_GET['maHeo'] ?? '';

    if (empty($maHeo)) {
        $_SESSION['error'] = "Không xác định được mã heo để xóa.";
    } else {
        $result = $heoModel->delete($maHeo);

        if ($result === true) {
            $_SESSION['success'] = "Xóa heo <strong>$maHeo</strong> thành công!";
        } else {
            $_SESSION['error'] = "Không thể xóa heo <strong>$maHeo</strong>!<br>$result";
        }
    }

    // Chuyển hướng về danh sách heo
    header('Location: index.php?url=heo');
    exit;
}

// Nếu không phải POST → load thông tin heo để hiển thị trang xác nhận
// (Giả sử bạn đã có code load $heo ở đây hoặc từ controller trước khi include view)
?>
