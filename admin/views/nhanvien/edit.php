<?php
$isEdit = isset($nv) && $nv; // true nếu là sửa, false nếu là thêm mới
        ...
        <form action="" method="post" class="bg-white rounded-2xl shadow-xl p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="HoTen" value="<?= htmlspecialchars($isEdit ? ($nv['HoTen'] ?? '') : ($_POST['HoTen'] ?? '')) ?>" 
                           required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Số điện thoại <span class="text-red-500">*</span></label>
                    <input type="tel" name="SDT" value="<?= htmlspecialchars($isEdit ? ($nv['SDT'] ?? '') : ($_POST['SDT'] ?? '')) ?>" 
                           required pattern="^0[3|5|7|8|9]\d{8}$" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">CMND/CCCD <span class="text-red-500">*</span></label>
                    <input type="text" name="CMND" value="<?= htmlspecialchars($isEdit ? ($nv['CMND'] ?? '') : ($_POST['CMND'] ?? '')) ?>" 
                           required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Ngày sinh</label>
                    <input type="date" name="NgaySinh" value="<?= htmlspecialchars($isEdit ? ($nv['NgaySinh'] ?? '') : ($_POST['NgaySinh'] ?? '')) ?>" 
                    ...
                    <div class="flex items-center gap-8 mt-3">
                        <label class="inline-flex items-center">
                            <input type="radio" name="GioiTinh" value="Nam" 
                                   <?= ($isEdit ? ($nv['GioiTinh'] ?? 'Nam') === 'Nam' : (!isset($_POST['GioiTinh']) || $_POST['GioiTinh'] === 'Nam')) ? 'checked' : '' ?> 
                                   class="w-5 h-5 text-emerald-600 border-gray-300 focus:ring-emerald-500">
                            <span class="ml-2 text-gray-700">Nam</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="GioiTinh" value="Nữ" 
                                   <?= ($isEdit ? ($nv['GioiTinh'] ?? '') === 'Nữ' : (isset($_POST['GioiTinh']) && $_POST['GioiTinh'] === 'Nữ')) ? 'checked' : '' ?> 
                                   class="w-5 h-5 text-emerald-600 border-gray-300 focus:ring-emerald-500">
                ...
                <div>
                    ...
                    <input type="text" name="DiaChi" value="<?= htmlspecialchars($isEdit ? ($nv['DiaChi'] ?? '') : ($_POST['DiaChi'] ?? '')) ?>" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="ViTri" value="<?= htmlspecialchars($isEdit ? ($nv['ViTri'] ?? '') : ($_POST['ViTri'] ?? '')) ?>" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Ngày vào làm <span class="text-red-500">*</span></label>
                    <input type="date" name="NgayVaoLam" value="<?= $isEdit ? ($nv['NgayVaoLam'] ?? '') : ($_POST['NgayVaoLam'] ?? date('Y-m-d')) ?>" 
                           required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>

                <div>
                    <label class="block text-gray-700 font-medium mb-2">Lương cơ bản (VNĐ)</label>
                    <input type="text" name="LuongCoBan" value="<?= $isEdit ? number_format($nv['LuongCoBan'] ?? 0, 0, ',', '.') : ($_POST['LuongCoBan'] ?? '') ?>" 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
                </div>
